...
// B (pinned bounce pipeline): extern declaration lives after the helper
// below; see copy_via_pinned_bounce.
...
bool ModelManager::stage_tensors_to_compute_backend(const std::vector<TensorState*>& states) {
    ...
    for (const auto& pair : states_by_staging_target) {
        ...
        for (auto& staged_tensor : staged_tensors) {
            ...
            // Swap tensor metadata so the managed tensor now points to
            // the VRAM buffer.  The async copy has already captured the
            // destination pointer, so the swap does not affect it.
